...
def train_on_filter(train_func, goal_datum, data_sources, bool_mask,**kwargs):
    """
    """
    goal_datum_resample = fs.select(goal_datum, bool_mask)
    data_sources_resample = [fs.select(d, bool_mask) for d in data_sources]
    return train_func(goal_datum_resample, data_sources_resample,**kwargs)

def test_on_filter(test_func, data_sources, bool_mask,**kwargs):
    """
    """
    data_sources_resample = [fs.select(d, bool_mask) for d in data_sources]
    return test_func(data_sources_resample,**kwargs)
