...
def lin_reg(goal_datum, data_sources,lin_reg_intercept=False):
    ...
    data_sources : list
        ...
        regression model.
    lin_reg_intercept: Boolean
        whether to include an intercept in the linear regression
    ...
    if lin_reg_intercept:
        ...
        reg.fit(series_matrix, goal_series)
    ...
    # Without intercept: y = A*x
        ...
        intercept = 0
        coefficients = np.linalg.lstsq(series_matrix, goal_series,rcond=-1)[0]
...
def lin_pred(data_sources, coefficients):
    ...
    data_sources : list
        ...
        regression model.
    coefficients : list
        Coefficients of the linear regression model. First element is the 
        intercept (potentially 0) and the second one contains the multipliers
    ...
    pred_series = np.dot(series_matrix, coef) + intercept
    return pred_series
