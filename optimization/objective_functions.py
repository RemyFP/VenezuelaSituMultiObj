...
def R_squared(problem, subset, **kwargs):
    ...
    """
    pred_series_CV = sa.pred_CV(problem.goal, subset,
                                problem.n_folds,
                                problem.bootstrap,
                                problem.lin_reg_intercept)['data']['values']
    goal_series = problem.goal['data']['values']
...
def R_squared_OOS(problem, subset, **kwargs):
    r"""OOS situational awareness objective function.

    A standard goodness-of-fit measure for determining how much variance
    in the goal time-series can be explained by given data sources already
    determined and now tested out of sample.
    ...
    """
    # Reorder list of data sources according to IDs alphabetical order
    subset_OOS = sorted(subset, key = lambda x: x['id']) 
    
    # Get coefficients of linear regression using in-sample data
    coef = sa.lin_reg(problem.goal,subset_OOS,
                      lin_reg_intercept=problem.lin_reg_intercept)
    
    # Get predicted time series out of sample
    subset_ids = [x['id'] for x in subset_OOS]
    OOS_subset = [datum for datum in problem.OOS_data if datum['id'] in subset_ids]
    OOS_subset = sorted(OOS_subset, key = lambda x: x['id']) 
    pred_series = sa.lin_pred(OOS_subset,coef)
    
    # Compute R-squared for out of sample data
    goal_series = problem.OOS_goal['data']['values']
    numerator = scipy.stats.tvar(goal_series - pred_series)
    denominator = float(scipy.stats.tvar(goal_series))
    rsquared = 1 - numerator/denominator
    return rsquared
